...
update.strengths <- function(dat, modelfunc){
  ...
  updated.strengths <- optim(strengths, modelfunc, dat=dat, players=players, method = "BFGS")$par
...
calibration <- function(analysis.df, name){
  ...
  scatter.smooth(x=xvals, y=yvals, main= "Model Calibration", xlab="Predicted Probability", ylab="Proportion of Correct Predictions")
...
# We have to consider surface models differently
  if (deparse(substitute(modelfunc)) == 'BT.result.time.surface' | deparse(substitute(modelfunc)) == 'BT.game.time.surface'){
    ...
    if (deparse(substitute(modelfunc)) == 'BT.result.time.surface'){
      model <- "result"
    } else if(deparse(substitute(modelfunc)) == 'BT.game.time.surface'){
      model <- "game"
    }
...
```{r}
#BT.result.model <- simulate(df, test.df, 2, BT.result, BT.result.winprob)
#BT.result.model
```

```{r}
#BT.game.model <- simulate(df, test.df, 2, BT.game, BT.game.winprob)
#BT.game.model
```

```{r}
#BT.result.time.model <- simulate(df, test.df, 2, BT.result.time, BT.result.winprob)
#BT.result.time.model
```

```{r}
#BT.game.time.model <- simulate(df, test.df, 2, BT.game.time, BT.game.winprob)
#BT.game.time.model
```

```{r}
BT.result.time.surface.model <- simulate(df, test.df, 2, BT.result.time.surface, BT.result.winprob)
BT.result.time.surface.model
```

```{r}
BT.game.time.surface.model <- simulate(df, test.df, 2, BT.game.time.surface, BT.game.winprob)
