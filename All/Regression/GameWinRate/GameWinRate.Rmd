...
for (i in(1:N)){
  ...
  winner.won <- subset(history, history$Winner == winner)
  winner.lost <- subset(history, history$Loser == winner)
  ...
  loser.won <- subset(history, history$Winner == loser)
  loser.lost <- subset(history, history$Loser == loser)
  ...
  X.gwr[i] <- winner.gwr - loser.gwr
  
  ymat.BT[i,1] <- 1 
}

BT.fit <- glm(ymat.BT ~ 0 + X.gwr, family=binomial(link=logit))
summary(BT.fit)
```
...
for (i in (1:M)){
  ...
  X.gwr.test[i] <- winner.gwr - loser.gwr
  
}
X.gwr.test <- as.data.frame(X.gwr.test)
X.test <- X.gwr.test

colnames(X.test) <- 'X.gwr'
...
calibration(rank.analysis.df, 'GameWinRate')
```
