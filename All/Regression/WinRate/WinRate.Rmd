...
```{r}
# Training Data Set
training.df <- read.csv(file="TrainingDataSet2.csv")
...
BT.fit <- glm(ymat.BT ~ 0 + X.wr, family=binomial(link=logit))
summary(BT.fit)
```
...
calibration(rank.analysis.df, 'WinRate')
```
...
```{r}
stukel(BT.fit, alternative = 'alpha1')
```
