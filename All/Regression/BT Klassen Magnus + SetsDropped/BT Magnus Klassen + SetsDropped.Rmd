...
for (i in(1:N)){
  ...
  recent <- tail(history,200)
  tourney <- recent[recent$Tournament == training.df$Tournament[i],]
  
  winner.matches <- subset(tourney, tourney$Winner == winner)
  winner.dropped <- sum(winner.matches$Lsets)
  
  loser.matches <- subset(tourney, tourney$Winner == loser)
  loser.dropped <- sum(loser.matches$Lsets)
  
  X.sd[i] <- winner.dropped - loser.dropped
  
  ymat.BT[i,1] <- 1 
}

BT.fit <- glm(ymat.BT ~ 0 + X.BT + X.sd, family=binomial(link=logit))
summary(BT.fit)
```
...
for (i in (1:M)){
  ...
  history <- test.df[1:i-1,]
  recent <- tail(history,200)
  tourney <- recent[recent$Tournament == test.df$Tournament[i],]
  ...
  X.sd.test[i] <- winner.dropped - loser.dropped
  
}
X.BT.test <- as.data.frame(X.BT.test)
X.sd.test <- as.data.frame(X.sd.test)
X.test <- cbind(X.BT.test,X.sd.test)
colnames(X.test) <- c('X.BT','X.sd')
...
calibration(rank.analysis.df, 'BTMK.SetsDropped')
```
