...
N <- nrow(training.df)
X.rank.pts <- rep(0,N)
ymat.rank.pts <- matrix(0, N, 2)

for (i in(1:N)){
  winner.rank.pts <- as.numeric(training.df$WPts[i])
  loser.rank.pts <- as.numeric(training.df$LPts[i])
  X.rank.pts[i] <- log(winner.rank.pts/loser.rank.pts)
  ymat.rank.pts[i,1] <- 1 
}

rank.pts.fit <- glm(ymat.rank.pts ~ 0 + X.rank.pts, family=binomial(link=logit))
summary(rank.pts.fit)
```

```{r}
M <- nrow(test.df)
X.rank.pts.test <- rep(0,M)

for (i in (1:M)){
  winner.rank.pts.test <- as.numeric(test.df$WPts[i])
  loser.rank.pts.test <- as.numeric(test.df$LPts[i])
  X.rank.pts.test[i] <- log(winner.rank.pts.test/loser.rank.pts.test)
}
X.rank.pts.test <- as.data.frame(X.rank.pts.test)
colnames(X.rank.pts.test) <- 'X.rank.pts'

rank.pts.fit.pred <- predict(rank.pts.fit, newdata=X.rank.pts.test, type='response')
```
...
# Predicted probability that the actual winner wins
    pred.win.prob <- rank.pts.fit.pred[i]
...
calibration(rank.analysis.df, 'rank.pts.klassen.magnus')
```