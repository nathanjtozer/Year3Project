...
for (i in(1:N)){
  ...
  X.GS[i] <- ifelse(training.df$Series[i] == 'Grand Slam', X.wr[i], 0)
  
  ymat.BT[i,1] <- 1 
}

BT.fit <- glm(ymat.BT ~ 0 + X.wr + X.GS, family=binomial(link=logit))
summary(BT.fit)
```
...
for (i in (1:M)){
  ...
  X.GS.test[i] <- ifelse(test.df$Series[i] == 'Grand Slam', X.wr.test[i], 0)
  
}
X.wr.test <- as.data.frame(X.wr.test)
X.GS.test <- as.data.frame(X.GS.test)
X.test <- cbind(X.wr.test,X.GS.test)

colnames(X.test) <- c('X.wr', 'X.GS')
...
calibration(rank.analysis.df, 'WinRate + GS')
```
