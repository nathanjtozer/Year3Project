...
```{r}
load('BT.stre.RData')
minimum <- min(stre$Mean)
stre$Mean <- stre$Mean + abs(minimum) + 1
...
for (i in(1:N)){
  ...
  winner.BT <- ifelse(training.df$Winner[i] %in% stre$Players, stre$Mean[which(stre$Players == training.df$Winner[i])], min(as.numeric(stre$Mean)))
  loser.BT <- ifelse(training.df$Loser[i] %in% stre$Players, stre$Mean[which(stre$Players == training.df$Loser[i])], min(as.numeric(stre$Mean)))
...
BT.fit <- glm(ymat.BT ~ 0 + X.BT, family=binomial(link=logit))
summary(BT.fit)
```
...
for (i in (1:M)){
  winner.BT.test <- ifelse(test.df$Winner[i] %in% stre$Players, stre$Mean[which(stre$Players == test.df$Winner[i])], min(as.numeric(stre$Mean)))
  loser.BT.test <- ifelse(test.df$Loser[i] %in% stre$Players, stre$Mean[which(stre$Players == test.df$Loser[i])], min(as.numeric(stre$Mean)))
...
colnames(X.BT.test) <- 'X.BT'

BT.fit.pred <- predict(BT.fit, newdata=X.BT.test, type='response')
```
