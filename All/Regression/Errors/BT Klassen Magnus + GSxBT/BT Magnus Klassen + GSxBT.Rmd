...
BT.fit <- glm(ymat.BT ~ 0 + X.BT + X.GS, family=binomial(link=logit))
summary(BT.fit)
```
...
X.BT.test <- cbind(X.BT.test, as.data.frame(X.GS.test))
colnames(X.BT.test) <- c('X.BT', 'X.GS')
...
calibration(rank.analysis.df, 'BT.Magnus.KLassen.GS')
```