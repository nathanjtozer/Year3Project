...
update.parameters <- function(dat, modelfunc){
  ...
  updated.parameters <- optim(initial.guess, modelfunc, dat=dat, players=players, method = "L-BFGS-B")$par
...
```{r}
sim.BT.result <- simulate(df, test.df, 1, BT.result, winprob)
sim.BT.result
```

```{r}
analysis(update.players(test.df), )
```